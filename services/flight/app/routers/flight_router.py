...
from app.schemas.passenger_schema import PassengerResponse 
from app.services.flight_service import get_flight_details_service
...
from app.schemas.flight_schema import FlightDetailsSchema
...
def get_flight(flight_id: int, db: Session = Depends(get_db)):
    flight = flight_service.get_flight_by_id(flight_id, db)
    if not flight:
...
def get_flight(flight_number: str, db: Session = Depends(get_db)):
    flight = flight_service.get_flight_by_number(flight_number, db)
    if not flight:
        raise HTTPException(status_code=404, detail="Flight not found")
    return flight
...
def get_passengers_by_flight_number(flight_number: str, db: Session = Depends(get_db)):
    passengers = flight_service.get_passengers_by_flight_number(flight_number, db)
...
def create_flight(flight: FlightCreate, db: Session = Depends(get_db)):
    return flight_service.create_flight(flight, db)

@router.put("/{flight_id}", response_model=FlightResponse)
def update_flight(flight_id: int, flight_data: FlightCreate, db: Session = Depends(get_db)):
    flight = flight_service.update_flight(flight_id, flight_data, db)
    if not flight:
        raise HTTPException(status_code=404, detail="Flight not found")
    return flight
...
def delete_flight(flight_number: str, db: Session = Depends(get_db)):
    result = flight_service.delete_flight(flight_number, db)
    if not result:
        raise HTTPException(status_code=404, detail="Flight not found")
